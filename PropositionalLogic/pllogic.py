...
for r in range(int(no_statements)):
	...
	if(validateinfix(infixchars)):
		...
		truth_assignments=make_truth_assignments(propconstants,truth_assignments)
		table=[]
		table.append([])
		for i in propconstants:
			table[0].append(i)
		table[0].append(str('CS-'+str(r+1)))
		for i in range(len(truth_assignments)):
			table.append([])
			value_expr=evaluatepost(output,truth_assignments[i])
			for j in propconstants:
...
print "Considering tautologies and self_contradictions to be equivalent to each other respectively"
...
print "Considering tautologies and self_contradictions to logically entail each other respectively"
print "The logical entailments are(other than the case of a C.S. logically entailing itself):"
